/**
 * SendPost API
 * Email API and SMTP relay to not just send and measure email sending, but also alert and optimise. We provide you with tools, expertise and support needed to reliably deliver emails to your customers inboxes on time, every time.
 *
 * The version of the OpenAPI document: 1.0.0
 * Contact: hello@sendpost.io
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';

/**
 * The ModelsEInvitation model module.
 * @module model/ModelsEInvitation
 * @version 1.0.0
 */
class ModelsEInvitation {
    /**
     * Constructs a new <code>ModelsEInvitation</code>.
     * @alias module:model/ModelsEInvitation
     */
    constructor() { 
        
        ModelsEInvitation.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>ModelsEInvitation</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/ModelsEInvitation} obj Optional instance to populate.
     * @return {module:model/ModelsEInvitation} The populated <code>ModelsEInvitation</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new ModelsEInvitation();

            if (data.hasOwnProperty('fromEmail')) {
                obj['fromEmail'] = ApiClient.convertToType(data['fromEmail'], 'String');
            }
            if (data.hasOwnProperty('toEmail')) {
                obj['toEmail'] = ApiClient.convertToType(data['toEmail'], 'String');
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>ModelsEInvitation</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>ModelsEInvitation</code>.
     */
    static validateJSON(data) {
        // ensure the json data is a string
        if (data['fromEmail'] && !(typeof data['fromEmail'] === 'string' || data['fromEmail'] instanceof String)) {
            throw new Error("Expected the field `fromEmail` to be a primitive type in the JSON string but got " + data['fromEmail']);
        }
        // ensure the json data is a string
        if (data['toEmail'] && !(typeof data['toEmail'] === 'string' || data['toEmail'] instanceof String)) {
            throw new Error("Expected the field `toEmail` to be a primitive type in the JSON string but got " + data['toEmail']);
        }

        return true;
    }


}



/**
 * @member {String} fromEmail
 */
ModelsEInvitation.prototype['fromEmail'] = undefined;

/**
 * @member {String} toEmail
 */
ModelsEInvitation.prototype['toEmail'] = undefined;






export default ModelsEInvitation;

